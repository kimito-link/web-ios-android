...
### 8.18 ワークフローの構文エラーは「CIが起動すらしない」を作り、テストが緑に見えたまま12日間実走しないことがある

- **症状**: GitHub Actions の `if:` 条件で `secrets` コンテキストを参照すると、actionlint で
  `if: ${{ secrets.XXX != '' }}` のような記述がエラーになる（**job/step どちらのレベルでも
  `if:` は secrets コンテキストを参照できない**。利用可能なコンテキストは
  env/github/inputs/job/matrix/needs/runner/steps/strategy/vars のみ）。この種の構文エラーが
  あるワークフローは **一度も起動せず**、GitHub 上のステータスは「実行履歴なし」や
  他ジョブの緑に隠れて気づきにくい。結果、依存する `e2e-smoke` 等のジョブが**丸ごと実走しないまま
  「テストは通っている」と誤認され続ける**。
- **原因**: `if:` 条件の secrets コンテキスト不可は GitHub Actions 仕様上の制約。ワークフロー
  YAML の構文エラーは push/PR トリガー自体を無効化するため、「テストが失敗して赤くなる」のではなく
  「テストが実行された形跡自体が残らない」形で不具合が隠れる。CIログや実行時間を見ずに
  「ワークフローファイルがある＝機能している」と判断すると見逃す。
- **直し方**:
  1. ワークフローを変更したら `actionlint` を必ず通す（ローカルまたはCI）。`if:` にsecretsを
     使いたい場合は、`env:` 経由で値を渡し、シェルスクリプト内で `[ -n "$VAR" ]` 判定に置き換える。
  2. 定期的に「対象ジョブが実際に実行された履歴があるか」（Actions タブの実行時間・実行回数）を
     直接確認する。0秒で終わる・実行履歴が極端に少ない・急に「起動しなくなった日」があるワークフローは
     構文エラーを疑う。
  3. 一度でも構文エラーで止まっていた期間があった場合、そのワークフローが検出するはずだった
     不具合が水面下に溜まっている前提で、直後の初回正常実行の結果を注意深く精査する
     （実際に kimitolink-linktree では修正直後の初回実走で2件の陳腐化バグが同時に表面化した）。
  - 出典: 2026-07-16、kimitolink-linktree の `playwright-smoke.yml`（`.github/workflows/playwright-smoke.yml`）。
    2026-07-04〜2026-07-16の12日間、`if:` 条件のsecretsコンテキスト誤用によりワークフローが
    起動失敗し続け、`e2e-smoke` ジョブが一度も実走していなかった。PR #186（コミット `8d43370`）で
    修正し、初回正常実行で `DEFECT-20260716-example-spec-stale-copy` と
    `DEFECT-20260716-signin-hydration-mismatch` の2件が同時に表面化した。
