...
- [ ] `site/_headers`に`/hub/*`への`X-Robots-Tag: noindex`が実際に反映されているか、
      デプロイ後に`curl -I https://kimito-skill.link/hub/`のレスポンスヘッダで確認した
- [ ] 生成された`site/hub/index.html`（実際のHTML）を目視し、KB本文・エラー実文言・
      triggersが出力されていない（パスと件数のみ）ことを確認した
- [ ] **Cloudflare Accessを`/hub/*`に設定するようユーザーに依頼した**（GUI操作、自動化不可。
      依頼済み・未設定の間はnoindexのみで実質公開状態であることをユーザーに明示した）
- [ ] 上記Access設定が完了したかをユーザーに確認した（未確認のまま作業完了報告をしない）
