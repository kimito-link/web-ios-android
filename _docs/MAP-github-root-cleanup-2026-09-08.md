...
## U. 第18弾: `_backups/kimitolink-line-line-fix-20260817.bundle`削除（★事故。教訓として記録）

### 到達点

**bundleは誤った分析に基づいて削除され、復旧不能。** 以下は事故の経緯記録。
...
**follow-greeting.tsは実際には今も必要であり、bundle削除は取り返しのつかない誤りだった。**
bundleは完全削除（ゴミ箱にも残らず）で復旧不能。line-botセッションのメッセージに
`follow-greeting.ts`全文が引用されていたため、コード自体は失われていないが、
一次資料（bundle）としては失われた。
...
- 担当セッション（line-bot）が同じデータへ独自にアクセスできる場合、司令塔の分析結果を
  鵜呑みにせず**独立に再検証する**ことが最後の防波堤になった。今回はline-bot側の
  再調査が事故の連鎖を止めた
