...
### ★追記（2026-09-10）: 「bundleは一次資料として失われた」も誤りだった

削除の数日後、無関係な作業（`kimitolink-line/`＝`Shudesu/line-harness-oss`の
クローン、他者OSSリポジトリの調査）でたまたま目に入ったブランチ名
`fix/follow-greeting-restore`をline-botセッションへ共有したところ、
そのブランチに`follow-greeting.ts`・`.test.ts`とコミット3件
（`5dda2e7`・`8c8469f`・`a0675e2`）が実在すると判明した（本セッションも
`git log`・`ls`で実測確認済み）。bundleにあった内容と同一（`8c8469f`は
bundleの`c5e0fc6`と同日・同内容）。

つまり「bundle削除＝一次資料の消失」という前提自体が誤りだった。
同じ内容が第三者OSSのクローンという別の場所に既に残っていた。
（結果的にline-bot側の実装の方が新しく判断も進んでいるため、取り込みは
不要と判断済み。この事実は「元がどうだったか」を後から確認する参照用。）

**教訓**: 「削除した＝失われた」と即断せず、実際に確かめる。この一件だけで
2回（最初の「follow-greeting不要」判定・その後の「一次資料喪失」判定）、
検証を飛ばして結論を出し、どちらも他セッションの実測で覆った。

## V. 第19弾: line-bot talent LP「push先誤解」報告の訂正（★事実誤認・訂正済み）

### 到達点

**訂正完了。** 以前（本セッション内）kimitolink-linktreeセッションへ「`7a272a7`・
`d361d72`は既に`origin/main`と完全同期している」と報告したが、これは誤りだった。

### 誤りの内容

`main`ブランチだけを見て`git log origin/main..HEAD`が空であることを確認し
「差分なし」と判定したが、実際にはこの2コミットは`main`ではなく
`feat/talent-lp-try-own-account`という別ブランチに存在していた。しかも
そのブランチ自体が`origin`にpushされていない（`git ls-remote origin`で0件）。
`origin`が自社リポジトリ（`kimito-link/linebot.git`）という部分の指摘は
正しかったが、「未反映のものは無い」という結論は誤りだった。

kimitolink-linktreeセッションが実測（`git log main..feat/talent-lp-try-own-account`・
`git ls-remote`）で誤りを発見し、訂正を受けて本セッションでも再確認・事実確認した。

### 教訓

- **「差分なし」の確認は、対象ブランチを正しく指定して初めて意味を持つ**。
  `main`を見て「同期済み」と判定したが、当のコミットが乗っているブランチ自体を
  確認していなかった。ブランチの存在確認（`git branch -a`）を飛ばして
  結論を出したのが根本原因
- kimitolink-linktreeセッションが`origin`へのpushを試みたところ自動モード分類器に
  ブロックされたとのことで、実行はline-bot担当セッションまたはユーザー本人の判断に
  委ねた（司令塔は代行実行しない）
