...
- **★いつやるか（2026-09-14 追記）: ビルドを回す前に済ませておく。** ビルドのアップロード・
  署名・CIの各ステップは App プライバシーの状態を一切見ないため、**全て成功したように見えて
  最後の submit だけが 409 で落ちる**。原因が提出の最終段にあるため発見が遅れる（実際に
  doin-challenge.com で、ビルドは正常アップロードされたのに submit だけ2回連続で 409 になり、
  ユーザーには「ストア一覧のアイコンが古いまま」という無関係な症状に見えた＝2026-09-14実損）。
  この項目はチェックリストの最後ではなく、**新規アプリを作った直後に一番先に済ませる**。
- **★「保存」と「公開」は別物**（強調）: ASC UI で8データタイプに回答して保存しても、
  右上の**「公開」ボタンがグレー→青に変わって初めて完了**（doin-challenge.com実測）。
  グレーのまま画面を離れると、回答内容自体は残るが「公開」されておらず、submit は409で弾かれる。
  保存ボタンを押した後、必ず「公開」ボタンの色を確認すること。
- **★所要時間の目安**: データタイプ1つにつき3問（用途 / ユーザーに紐づくか / トラッキング）
  あり、1タイプあたり6画面のウィザードを通る。doin-challenge.com（8タイプ）で実測**約40分**。
  新規アプリのスケジュールにこの時間を見込んでおくこと。
- **★自動化できる部分**: Android の Data Safety（`store-assets/play/data-safety.csv`）を
  既に用意していれば、`templates/scripts/asc-generate-privacy-answers.mjs` で
  ASC UI に入力すべき内容を根拠つきのチェックリストとして生成できる（自動入力ではなく、
  人間の入力を1回で終わらせるための下書き）。広告/計測SDKが検出されると
  「トラッキング=いいえ」を機械的に断定しない警告が出る。
- **★lint での検知**: `lint-pre-submission.mjs`（CHECK 10〜13、`asc-readonly-checks.mjs`）は、
  ASC API の JWT 認証では公開状態を直接読めない（iris は web セッション専用）ため、
  `app.config.json` の `stores.iosPrivacyPublishedAt`（YYYY-MM-DD）に人間が公開した日付を
  宣言しているかで判定する。**宣言が無ければ fail**（2026-09-14以前は warn で緑のまま
  素通りしていた＝「検査していない」が「合格」と同じ緑になっていたバグ。doin実損を受けて
  fail-closed に変更）。公開したら忘れずに宣言すること。
- **直し方**（ASC UI 手動・API は構造が複雑なので UI 推奨）:
  1. ASC → アプリ → 左メニュー **「アプリのプライバシー」**。
  2. データ収集: 個人を特定・追跡しないツールなら **「データを収集していません」**。
     データを収集するなら、Android の data-safety.csv があれば
     `asc-generate-privacy-answers.mjs` の出力を下書きに使う。
  3. **プライバシーポリシーURL** を入力（例 `https://<domain>/privacy/`・ページが実在すること）。
  4. **右上の「公開」ボタンを必ず押す**（"published answers" が要件＝保存だけでは不可。
     ボタンがグレー→青に変わったことを確認する）。
  5. 「○分前に公開済み」と出れば OK。`app.config.json` の `stores.iosPrivacyPublishedAt` に
     その日付（YYYY-MM-DD）を設定する（lint がこの宣言を見て fail/warn を判定する）。
