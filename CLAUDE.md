...
**スクリーンショット異常は、アプリ表示異常とは別の事象として扱う。DOM・CSS・rAFが正常なら、
アプリコードを変更する前にcapture経路を疑う。**

> 実損: `site/index.html`のCSS修正後、Browser paneでスクロール後にスクリーンショットを
> 撮ると画面全体が真っ黒になった。原因をCSS側だと決めつけて修正を重ねる前に、
> `document.visibilityState`（→`visible`）・`document.hasFocus()`（→`true`）・
> `requestAnimationFrame`の発火（→`fired`）・`get_page_text`（→正常取得）を実測したところ
> 全て正常で、アプリ側の描画不具合はほぼ除外できた。再試行したところ同じ操作で
> 正常なスクリーンショットが撮れ、Browser pane側のcapture経路の一過性障害だったと判断できた
> （★「確定」とは言い切れない。DOM/CSS/rAFが正常という外部観測だけでは、Claude内部の
> どの層が原因だったかまでは証明できないため、言い過ぎない）。

**判定の型（5段階、上から順に確認する）:**

1. **実装確認**: ソースに意図した値が存在するか（`grep`等でコードを読む）
2. **実行時確認**: `getComputedStyle`等で実際にブラウザが評価した値を確認する
3. **機械的品質確認**: コントラスト比などを数値で検証する（WCAG基準等の客観指標に照らす）
4. **視覚確認**: 実ブラウザで人間が見る状態（スクリーンショット）を確認する
5. **本番反映確認**: デプロイ先とカスタムドメインの内容一致を確認する
   （`templates/scripts/deploy-cloudflare-pages.mjs`の反映確認機構、2026-09-10実装）

★4（視覚確認）で異常が出たら、直ちに5段階のうち**1〜3が既に正常だったか**を振り返る。
1〜3が正常なのに4だけ異常なら、対症療法でコードを書き換える前に、まずobservability自体
（スクリーンショット取得経路・ブラウザのフォーカス/可視性状態）を疑う。

★これは既存の「基準③完膚なきまでの裏取り」の具体化であり、新しい基準を増やす話ではない。
「動くはず」で止めない、の裏返しとして「壊れて見える」で即断しない、を明文化しただけ。

**同じ思想の対（このキットの既存知見と対になる）:**
> HTTP到達 ≠ ページ存在 ≠ 期待画面 ≠ 操作成功
> ★これに加えて: **DOM正常 ≠ 描画正常 ≠ capture正常**

観測のどの層（ネットワーク到達／DOM／描画／スクリーンショット取得）で確認したかを
常に区別する。1つの層が緑でも、他の層まで緑とは限らない。
