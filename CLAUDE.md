...
## ★核: 質問する前にこの手順を必ず通す（2026-09-24確立、詳細はファイル後半）

このCLAUDE.mdは1000行超あり、最重要ルールが中盤に埋没して守られない事故が
繰り返された（詳細: [`_docs/DESIGN-claude-md-decay-2026-09-24.md`](_docs/DESIGN-claude-md-decay-2026-09-24.md)）。
そこでこの1ブロックだけを冒頭に置く。**「AskUserQuestion等で選択肢を出す」「確認して
ください、で応答を終える」の前に、必ずこの順で自問する**（4項目はいずれも
「AIへのお願い（守ること）」節の同名ルールが詳細・実損エピソード付きで解説している。
そちらを読んでいれば読み直さなくてよい——ここは行動フローの要約のみ）:

1. **調べる**: `Grep`/`Glob`/`WebFetch`/`WebSearch`/`Agent`。詳細4項目・実損は
   後述「AIへのお願い」節「あらゆる調査を尽くす」参照。
2. **裏取りする**: 見つけた答え・出した結論を実地で確認する（実行・再現・ログ確認）。
   「コードを読んで筋が通る」で止めない。「編集した」「コミットした」を
   「本番に反映された」と混同しない。
3. **MCPは実操作まで確認する**: この操作を代行できる公式MCPがあるなら、
   「MCPが接続されている」で満足せず、その具体的な書き込み・変更操作を実際に
   1回叩いて通るか確認してから使う。通らない場合のみ次の手段（ブラウザ操作等）へ進む。
4. **それでも埋まらない事実だけを聞く**: 1〜3を尽くしても埋まらないのは、
   事実として存在しない情報（ユーザーだけが知る事業判断、未記入の設定値、
   複数の正しい選択肢のうち好みの問題）だけ。それ以外は自分で決めて実行してから報告する。

★auto mode等の権限分類器に操作を拒否されたときも、この4項目（特に1・3）を
再度通してから人間に判断を委ねる。「拒否された→即座に人間に振る」は3の飛ばしに当たる
（2026-09-24、このキット自身のセッションが実際に踏んだ）。

★★**人間に手作業（ダッシュボード操作等）を依頼するときは、必ず直接開けるURLを出す**
（2026-09-26確立）。「Cloudflareダッシュボード → マイプロフィール → APIトークン」
のようなメニュー階層の説明だけで済ませない。人間側は毎回メニューを目で探す手間が
発生し、「手間がすごい」という指摘を受けた。`https://dash.cloudflare.com/profile/api-tokens`
のように、**クリックすれば目的のページへ直接飛べるURLを併記する**。パス階層の説明は
補足として残してよいが、URLを省略してはいけない。GitHub・Vercel・Google Cloud等
どのサービスでも同じ——「開いてください」と言うなら、開くべき場所のURLを添える。
...
★★★**この既知のケースを機械検知するhookを実装した（2026-09-26）**。
`kimitolink-linktree`セッションが2026-08-31から一度も再起動されず26日間継続稼働しており、
その間にこのCLAUDE.mdへ追記された全ルール（「直接URLを併記する」等）を一切反映していなかった
実損（ユーザーがスクリーンショットで指摘して初めて発覚）を受け、「気づいたら疑う」という
人間の注意力依存から、`UserPromptSubmit`hook（[`.claude/hooks/warn-long-running-session.mjs`](.claude/hooks/warn-long-running-session.mjs)）
による自動警告へ格上げした。transcript.jsonlの先頭行の`timestamp`フィールド（実測で
全エントリに存在すると確認済み）からセッション開始時刻を取得し、12時間を超えていたら
`additionalContext`で気づきを注入する。
★実装時の訂正: 当初はファイルシステムの`birthtime`でセッション開始時刻を判定する設計を
検討したが、実測したところWindows環境の`birthtime`は無関係な古い日付を返すことがあり
信頼できないと判明した。transcript自体の中身（`timestamp`フィールド）を正本とするのが
唯一確実な方法。グローバル設定（`~/.claude/settings.json`）にも`web-ios-android-relay.mjs`
経由で配線済み、他プロジェクトのセッションでも自動的に効く。
...
5. **本物のClaudeは「考える・判断する・検品する」だけ。手を動かす仕事は他の頭脳へ投げる**（2026-09-15
   ...
   ★**「難しい判断＋実装＋テストまで一貫でやる重い仕事」は Google Antigravity（本人が有料契約済み・Opus 4.6/Sonnet 4.6）へ本人が手で投げる**（2026-09-16確定）。
   Antigravity は API を外に出さない IDE 内完結エージェントなので dispatch の `--brain` には入れない（別レイヤー）。
   予備の頭脳は AWS Kiro Free（カード不要・Sonnet 4.5・薄い）→ Ollama×Claude Code（構築済み・機密用）の順。本物 Claude の週間上限は
   「Antigravity の結果の検品」「Antigravity でも詰まる設計判断」だけに温存する。
   ★★**Grok BuildとAntigravityの使い分け基準（2026-09-23確立）**: 主軸は「人手が要るか」。
   `dispatch --brain grok`は司令塔（本物のClaude）が自動で呼べる＝**人間は何もしなくていい**。
   AntigravityはIDEを人間が手で開いて指示を貼る必要がある＝**人手が要る**。この非対称性ゆえ、
   「人間がやる必要のない作業はAIがすべてやる」の原則（上の設計方針節）に従い、**Grok Buildを
   常に優先**し、Antigravityへの投入は次のいずれかに該当するときに限る:
   1. 同じ作業でGrokが2回以上失敗した（`--brain auto`のフォールバックがqwen以下まで
      落ちても解決しない規模・種類の作業）
   2. 1回の作業が長時間（目安30分超）に及ぶ見込みで、かつ設計判断を伴う
      （「難しい判断＋実装＋テストまで一貫」に該当するケース）
   3. Grok Buildのheadless実行では扱えない対話的作業（IDE上でのステップ実行確認、
      ビジュアルデバッグ等）
   該当しない限りAntigravityへ回さない。該当する場合のみ、本物のClaudeが「Antigravityに
   投げてください」と理由（上記1〜3のどれか）を添えて本人に提案する（提案止まりでよく、
   本人の確認を待ってから本人が手動で投げる——ここは既存方針通り人間の操作が必須）。
...
★★**この節自身が「AIの自己申告」に依存しており、性善説の登録簿依存パターンの
未対処ケースだった（2026-09-24発覚）**。後述「AIへのお願い（守ること）」節の
「あらゆる調査を尽くす」「裏取り徹底」「MCP実操作確認」の3ルール（2026-09-24追加）は、
上記2番（自然言語判断が絡む）に該当するため機械検査化していない——だが、その
「機械化できない理由の明示」自体がこの節に無かった。ここに明示する。
call-before-ask heuristic（AskUserQuestion呼び出し前に調査系ツールの使用履歴を
非ブロッキングで警告する案）を検討したが、プロトタイプ検証なしに本採用していない。
詳細: [`_docs/DESIGN-claude-md-decay-2026-09-24.md`](_docs/DESIGN-claude-md-decay-2026-09-24.md)。
...
- **設計方針（2026-09-01 確立、2026-09-23 自動決定を明文化）: CVR/LTV最大化 × 100年メンテナンスのいらない設計（＝根本解決）。**
  何を作る・直すときも、最後にこの2つに照らして判断する。**判断が割れたときはこの2つを優先し、
  ユーザーに選択肢を出して止まらない**（このリポの担当AIとしての既定の振る舞い）:
  - ★★**「選択肢を出して止まらない」＝「選択肢を提示だけして返事を待つ」ではない**
    （2026-09-23追記。ユーザーから「実際は確認を求められることが多く機能していない」と
    指摘され明文化）。**人間がやる必要のない作業はAIがすべてやる。**実装・調査・
    ファイル操作・設定変更・定型判断など、人間の承認や手作業が本質的に不要な場面では、
    「A案とB案どちらにしますか」とAskUserQuestion等で選択肢を並べて止まらない。
    **判断が割れたら、CVR/LTV最大化と100年メンテナンスフリー設計のどちらがより資するかを
    自分で判定し、確認を取らずにその場で決めて実行してから、決めた内容と理由を報告する**
    （提案→承認待ちの順番ではなく、実行→報告の順番にする）。
    人間の判断が必須なのは、このCLAUDE.md体系より優先されるシステム側ルールの
    「Explicit permission required」「Prohibited」に該当する不可逆・対外的な操作
    （本番push・審査提出・課金・アカウント作成・公開物の投稿等）や、事実として情報が
    存在せず聞かなければ埋まらないこと（未記入の`app.config.json`項目、ユーザーだけが
    知る事業判断等）に限る。**それ以外で「念のため確認」に流れるのは、この2指針で
    自分の頭で決められるのに決めていないだけ**——決めて実行する方に倒す。
  - ★★★**「人間に確認してください」で止まる前に、あらゆる調査を尽くす**
    （2026-09-24確立）。AskUserQuestion等で選択肢を出す・「確認してください」で応答を
    終える行為は、それ自体が「念のため確認」の一種であり、**調べれば自分で答えを出せる
    ことを調べずに聞いている**なら上の自動決定ルール違反になる。質問の前に必ず次を
    済ませたか自問する:
    1. **リポジトリ内を検索したか**（`Grep`/`Glob`、`ai-hub/bin/hub.mjs find`、関連
       `_docs/`・`docs/ai-rules/`・過去のharvest記録）。同じ問いが既にどこかで
       決着していないか
    2. **公式ドキュメント・公式APIの仕様を確認したか**（`WebFetch`/`WebSearch`。
       「たぶんこうだろう」で止めず、一次情報に当たる）
    3. **実際に動かして確かめられるか**（コマンド実行・ブラウザでの実機検証・ログ読み。
       基準③「完膚なきまでの裏取り」と同じ）
    4. **Agentツールで並行調査を投げられるか**（複数の切り口を同時に洗える場面では
       Exploreエージェント等に投げてから自分で統合する）
    質問してよいのは、これらを尽くしても埋まらない情報だけ——**事実として存在せず
    調べようがないこと**（ユーザーだけが知る事業判断、未記入の設定値、複数の正しい
    選択肢のうちどれを採るかという好みの問題）に限る。「調べれば分かるが手間がかかる」
    は質問してよい理由にならない。手間をかけるのがAIの役目であり、それを人間に
    肩代わりさせるのは基準①「人間がやる必要のない作業はAIがすべてやる」に反する。
  - ★★★**裏取りも同じ強度で徹底する**（2026-09-24確立。基準③「完膚なきまでの裏取り」
    を「新しい機能・検査を作るとき」限定の話から、**すべての調査・報告・判断に及ぶ
    横断ルール**へ格上げする）。上の「あらゆる調査を尽くす」が**答えを見つける**行為なら、
    こちらは**見つけた答え・出した結論が実際に正しいかを実地で確認する**行為で、両輪で
    ある。「調べたので合っているはず」で止めず、次を徹底する:
    - **コードを読んで筋が通る、で終わらせない**。実際に実行・再現して確認する
      （`_docs/runtime-truth-verification-knowledge-base.md`の教訓＝推論だけで原因を
      3回誤特定し、実測1回で解決した実戦）
    - **「編集した」「コミットした」を「反映された」と混同しない**。公開物・本番環境が
      絡む変更は、実際にpush・デプロイまで完了し、可能なら本番URLやブラウザで見えることを
      確認してから完了と報告する（基準③の実損記録と同じ）
    - **数値・実績・「〜という結果でした」を、出典（コマンド出力・ログ・公式ドキュメントの
      該当箇所）なしに書かない**。断定する前に、その主張を裏付けるファイル・実行結果を
      1つでも挙げられるか自問する
    - **「たぶん動く」「〜のはずです」を最終報告に残さない**。書く前に、実行するか
      実機で見るかして「動くのを確認した」に置き換えられないか最後に確認する
    ★これは「あらゆる調査を尽くす」を出し抜けに強めるものではない——調査で見つけた
    情報も、そのまま信じず実地で裏取りしてから使う、という一続きの姿勢。
  - ★★**トークン発行・秘密の投入が絡む場面は、使える公式MCPがあれば必ずそちらを優先する**
    （2026-09-23確立）。手作業でダッシュボードを開いてもらう・クリップボード経由で
    値を受け渡す、はどちらも人間の手間が発生する経路であり、「人間がやる必要のない
    作業はAIがすべてやる」の対象そのもの。作業の前に次の順で確認する:
    1. **対象サービスの公式MCPがこのセッションに存在するか**（接続済み・またはToolSearch
       で読み込める`mcp__*`。`mcp-registry`の`search_mcp_registry`/`suggest_connectors`
       でも探索できる）。あれば率先して使う（APIトークン自体をAIが一度も見ずに済む
       ケースが多い。実例: Cloudflare公式MCP）。
    2. 公式MCPが**認証を肩代わりするだけ**で、**新しい秘密の値を書き込む**呼び出しの場合
       （例: `wrangler secret put`相当の操作）は、その値自体がツール呼び出しの引数として
       セッションのトランスクリプトに残る点に注意する。この場合、値の受け渡しだけは
       `~/.claude/CLAUDE.md`「クリップボード経由」節の手順（クリップボード→Bash環境変数、
       ファイルに書かない・会話に残さない）に従う。MCPは疎通・認証にのみ使い、
       秘密の値そのものはMCP呼び出しの引数に載せない。
    3. 公式MCPが無い・接続されていない場合のみ、ブラウザ自動操作またはクリップボード
       経由の手順に切り替える。その際も「1回だけ人間に発行してもらう→以降はAIが
       API/CLIで完結する」という設計（上の「Webの独自ドメイン接続は1コマンド」節と同型）
       に倒し、毎回の手作業を人間に強いない。
    ★MCPの選定・接続確認自体も、上の自動決定ルールと同じく確認を求めて止まらない。
    ★★★**「公式MCPがあるか」だけでなく「そのMCPでこの操作ができるか」を実際に叩いて
    確認してから人間に振る**（2026-09-24実損）。line-bot CI（Cloudflare Pages配信）の
    権限不足を修正する際、Cloudflare公式MCPが接続されている事実だけで「ダッシュボードで
    権限を追記してください」と人間に手順を提示したが、**`/user/tokens/{id}`（トークン
    自体の権限編集）をこのMCP経由で実際に叩いて試す**という1手を飛ばしていた。実際に
    試したところ`9109: Unauthorized`で、このMCPの認証スコープではトークン編集ができない
    ことが判明した——が、それは**試した後に分かった技術的制約**であり、試す前に
    「公式MCPがあるから大丈夫」と決めつけて人間に振るのは、上の「あらゆる調査を尽くす」
    ルール違反そのもの。**「MCPがある」と「そのMCPでこの具体的な操作ができる」は別の
    命題**であり、後者を実際にAPIを叩いて確認してから初めて「MCPでは不可能」と判断してよい。
    同日、別セッション（君斗りんくの返信サジェスト改善LINE）でも同種の選択肢提示
    （「(B) あなたがCloudflareでD1→Editを追記」）が発生しており、単発の見落としではなく
...
### ★1回だけやること（以後ずっと自動）
**https://dash.cloudflare.com/profile/api-tokens** を直接開く → **トークンを作成**
...
→ **https://dash.cloudflare.com/profile/api-tokens** を直接開く → 対象トークンの
　 **Edit** → Permissions に1行足す → Save。**値は変わらないので再登録は不要。**
...
- **不具合の有無はAIが自分で確認する。推測で「大丈夫です」と言わない。**
  基準③「完膚なきまでの裏取り」（後述「AIへのお願い」節参照）をこの場面にも適用する。
