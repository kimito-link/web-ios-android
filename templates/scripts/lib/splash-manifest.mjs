 ...
 *   release CI がこれらの検査を呼ぶのはこのキット側であり、正本を分けると
 *   **まさに今回直した「5リポ分岐」と同じ構造を1段上で再現する**。
 *   よってキット側に統合し、`github/splash/` は非正本（試作の記録として残置）に降格した。
 *
 * ★2026-08-25、画像生成そのものは公式 `@capacitor/assets` に移した。
 *   ここが配るのは「公式が面倒を見ない所を確かめる検査」だけで、生成器は含まない。
 */
