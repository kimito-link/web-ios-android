...
| `shared/ai-triage/` | **AI応答のJSON修復(`json-repair.mjs`)・返信トーン定義(`draft-tone.mjs`)の共有モジュール**(外部依存ゼロの純粋関数のみ)。henshin-hisho/gmail-secretary-extensionの`triage.js`・`draft-gen.js`に別々にコピーされ、コピー後に片方だけ修正が入って乖離していた実損を受けて2026-09-08設計・新設。現時点では両リポへの配布(import差し替え)は未実施(別Issue)。詳細は [`../_docs/DESIGN-ai-triage-shared-module-2026-09-08.md`](../_docs/DESIGN-ai-triage-shared-module-2026-09-08.md) | 各モジュール単体で`node <file> --selftest`実行可。配布時は無改変でimportする契約 |
...
| [`web/site-chrome/`](web/site-chrome/README.md) | **複数ページ静的サイト用、共通ヘッダー・フッターの金型**(ビルドレス・JS実行時注入)。ページごとのheader/footerコピペ・相対パスのズレ・直し漏れを防ぐ。web-ios-androidキット自身のsite/が実運用。2026-09-01: この金型が無かったため他プロジェクト(line-bot/apps/lp)でheader/footer未共通化事故が実際に起きた反省で追加。2026-09-03: Core(`core/site-chrome.js`/`.css`、変更禁止)とConfig(`config/schema.json`準拠のJSONから`generator/generate-site-chrome-consumer.mjs`で生成)を分離し、SHA-256でCURRENT/DRIFTEDを機械判定できる設計に再編(3プロジェクトで実証済み) | `config/example.json`を対象サイトの`site-chrome.config.json`にコピーしてブランド名・ロゴ・navItems・navCollapseAt・色を書き換え、`generator/generate-site-chrome-consumer.mjs --config <path> --out <対象ディレクトリ>`を実行。手でJS/CSSを編集しない |
| `scripts/rollout-plan.mjs` / `scripts/rollout-preflight.mjs`(**web-ios-androidキット自身の`scripts/`配下**、テンプレではなく運用ツール) | site-chrome等の配布可能コンポーネントを、github/配下の全リポジトリへread-onlyで一括診断する。`rollout-plan.mjs`は対象判定(Applicability)と導入状況(CURRENT/MISSING/DRIFTED/UNKNOWN)を、`rollout-preflight.mjs`はapply前のCSP互換性(meta CSP + `_headers`/`vercel.json`のhosting CSP)をそれぞれ機械判定する。判定ロジックの正本は`scripts/lib/component-rollout.mjs` | 新しいコンポーネントを追加する場合は`scripts/lib/component-rollout.mjs`の`COMPONENT_REGISTRY`に登録するだけ(scanner本体は変更不要) |
...
| `scripts/lib/instrument-proof.mjs` + `scripts/check-instrument-proof.mjs` + `scripts/record-instrument-proof.mjs` | 「検査がexit 2を持っているか(所有)」ではなく「実対象で赤・緑の両方を観測した記録があるか(証明)」を見る証明3点台帳(2026-09-02、Fable 5.1へ「大元の診断計器をパワーアップして」と依頼した設計の実装。`_docs/instruments/README.md`掟⑬)。台帳(`.instrument-proof.json`)は`run-instruments.mjs --report`実行のたびに`record-instrument-proof.mjs`が自動更新。ソースの`sourceHash`が変われば過去の緑を無効化する仕組みで「直した検査が実機で緑のままか」を機械判定。90日超えて一度も赤を経験していない検査はinconclusiveで明示(掟⑪の機械化)。**限界**: 台帳への書き込み自体は手で偽装可能。`check-instrument-proof.mjs --check`の判定結果はまだ`run-instruments.mjs`の集約exitに混ぜていない(導入直後は影響範囲が大きすぎるため個別実行から開始) | **無改変**(`run-instruments.mjs`から自動配線済み。個別確認は`node scripts/check-instrument-proof.mjs --check`) |
...
| `scripts/verify-responsive-runtime.mjs` | 複数ビューポート幅でPlaywrightを巡回し、横スクロール・要素はみ出し・テキスト省略・ナビ/ハンバーガーの排他性を実ブラウザで機械判定(2026-08-31、malwarecheck.siteの`check-header.mjs`/`check-responsive.mjs`を統合・大幅汎用化して逆輸入)。既存の`verify-responsive-design.mjs`（CSS静的解析のみ、実ブラウザ判定はresponsive-checkスキル/Playwrightに委ねると明記済み）を補完する、CIで自動実行できる実測ゲート。1024pxと1040px両方を測る(スクロールバーでの境界誤判定を回避)等、移植元の実測知見を踏襲。`--config`で対象幅・ナビ検査要否を指定。実データ裏取り: best-trust.bizの実ページに対して実行し横スクロール崩れを実際に検出・動作確認済み | **無改変**(対象URL必須引数。CIには未配線、`@playwright/test`が無い環境ではinconclusiveで停止) |
...
| `scripts/clerk-auto-login.mjs` | **★人の手なしでログイン状態を作る**。Clerk の sign-in token(`ticket` strategy)で セッションを張る。headless で動き、patchright も永続プロファイルも不要。録画・E2E・スクショから「人がXログインする」工程が消える。`@clerk/testing` は identifier=メール前提で、**X/Appleログインのユーザーにはメールが無い**ため使えない（実測） | **要改変**(`DEMO_X_USERNAME` を自アプリの公式アカウントに) |
