...
### 1c. Grok Build と Antigravity の使い分け（2026-09-23確立、正本は`CLAUDE.md`セッション開始時アクション5番）

Grok Build（`--brain grok`）と Antigravity は、どちらも「難しい実装」を任せられる先頭集団の頭脳だが、
**呼び出し方が非対称**: Grok Buildは司令塔（本物のClaude）が`dispatch.py`で自動的に呼べる＝人間は
何もしなくていい。Antigravityは本人がIDEを手で開いて指示を貼り付ける必要がある＝人手が要る。

この非対称性を主軸に、**Grok Buildを常に優先**する。Antigravityへ回すのは次のいずれかに該当するときだけ:

1. 同じ作業でGrokが2回以上失敗した（`--brain auto`のフォールバックがqwen以下まで落ちても解決しない）
2. 1回の作業が長時間（目安30分超）に及ぶ見込みで、かつ設計判断を伴う
3. Grok Buildのheadless実行では扱えない対話的作業（IDE上のステップ実行確認、ビジュアルデバッグ等）

該当する場合のみ、司令塔が理由（上記1〜3のどれか）を添えて本人にAntigravity投入を提案する
（提案止まり。本人の確認を待ってから本人が手動で投げる）。

★**新しい頭脳（Codex等）を追加する前に、車輪の再発明をしない**（CLAUDE.md「新しい機能・
検査を作るときの4つの基準」§2と同じ考え方をこの運用にも適用）。「もう1つ頭脳を増やせば
速くなるのでは」と思っても、まず上表のGrok Build（先頭集団の賢さ・8秒実測・人手ゼロ）で
足りないかを確認する。既存の`--brain`ラインナップ（grok/qwen/oc/cf/local/gemini/groq）で
賄える仕事に、契約や配線が要る新しい頭脳を増やさない。増やす価値があるのは、既存のどれも
向いていない仕事の型が実際に見つかったときだけ。

### 1d. Workflowツール（dynamic workflow）は使わないのが既定（2026-09-25確定）

Claude Code公式のマルチエージェント編成機能。単語"workflow"で誤発火し得る上、
**Claude週間上限を体数分消費する**——「実装は他頭脳へ」という上のマルチ頭脳運用の
思想と逆方向に財布を動かすため、標準採用しない（会議＋Fable設計で判定。
`_docs/DESIGN-dynamic-workflow-adoption-2026-09-25.md`）。`/config`の
「ダイナミックワークフロー」はOFF推奨（`enableWorkflows: false`。誤発火防止は
単語トリガーでなく機能ごとの無効化）。

使ってよいのは次の4条件を**すべて**満たす時だけ: ①仕事の中身が判断・検品である
②3体以上のサブエージェントに依存関係がある③中断→再開が実際に必要な長さ
④ユーザーが今回明示的に「Workflowツールで」と言った。

Workflowで起動したサブエージェントも`.agent/coord.md`の対象——正本（commit/push）は
司令塔だけが動かす。`/loop`は判断が要るときだけ使い、要らない見張りは決定的ポーラ
（`job-runner.py`・poll workflow）に任せる。
